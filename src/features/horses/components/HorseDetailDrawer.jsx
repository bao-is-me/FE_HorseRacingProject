    ...
    <div className="horse-overlay drawer-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <aside className="horse-drawer" role="dialog" aria-modal="true" aria-labelledby="horse-detail-title">
        <header className="horse-drawer-hero">
          <HorseImage src={horse.imageUrl} alt={horse.name} size="hero" />
          <div>
            <span>Horse detail</span>
            <h2 id="horse-detail-title">{horse.name}</h2>
            <p>{horse.breed || "Breed not provided"} · {horse.color || "Color not provided"} · {horse.age ?? "—"} years</p>
            <div className="horse-status-row">
              <StatusPill>{horse.status || "Unknown"}</StatusPill>
              {horse.derivedStatus && <StatusPill>{horse.derivedStatus}</StatusPill>}
            </div>
          </div>
        ...
        <div className="horse-detail-content">
          <section>
            ...
            <dl className="horse-detail-grid">
              ...
              <div><dt>Created at</dt><dd>{formatDateTime(horse.createdAt)}</dd></div>
          ...
          <section>
            ...
            <div className="horse-owner-card">
              <strong>{horse.owner?.name || "Owner information unavailable"}</strong>
              <span>{horse.owner?.email || horse.ownerId}</span>
              {horse.owner?.phone && <small>{horse.owner.phone}</small>}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="horse-relation-list">
              {horse.entries.length ? horse.entries.map((entry) => (
                <article key={entry.id}>
                  <div><strong>Race {entry.race?.raceNumber}</strong><StatusPill>{entry.status}</StatusPill></div>
                  <p>{entry.race?.tournament?.name || "Tournament unavailable"}</p>
                  <small><MapPin size={13} /> {entry.race?.racecourseName || "Racecourse unavailable"} · Gate {entry.gateNumber ?? "—"}</small>
                  <small>Jockey: {entry.jockey?.name || entry.jockeyId} · {formatDateTime(entry.race?.startTime)}</small>
          ...
          <section>
            ...
            <div className="horse-reward-summary">
              <div><span>Total rewards</span><strong>{formatCurrency(horse.totalRewards)}</strong></div>
              <div><span>Reward count</span><strong>{horse.rewards.length}</strong></div>
            </div>
            <div className="horse-relation-list">
              {horse.rewards.map((reward) => <article key={reward.id}><div><strong>{reward.prizeType}</strong><b>{formatCurrency(reward.amount)}</b></div><small>{reward.distributedAt ? formatDateTime(reward.distributedAt) : "Not distributed"}</small></article>)}
            </div>
          </section>
        </div>
      </aside>
